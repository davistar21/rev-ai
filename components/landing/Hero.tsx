...
export function Hero() {
  ...
  return (
    <GlassContainer className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 md:px-8 py-20 overflow-hidden">
      <motion.div 
      ...
      >
        ...
        <p className="text-xl opacity-80 mb-10 max-w-2xl mx-auto leading-relaxed">
          Analyze Interswitch tx → Revenue forecast + Fraud alerts + Nano-credit score implicitly tied exclusively to Groq inference logic.
        </p>
        
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/login">
            <GlassButton size="lg" variant="primary" className="w-full sm:w-auto text-lg px-8 hover:scale-105 transition-transform">
              Go to Merchant Hub <ArrowRight className="w-5 h-5 ml-2" />
            </GlassButton>
          </Link>
          <a href="#api-docs">
            <GlassButton size="lg" variant="secondary" className="w-full sm:w-auto text-lg px-8 hover:scale-105 transition-transform">
              Explore API
            </GlassButton>
          </a>
        </div>
